...
@Service
public class DoctorMediaForHospitalService {
	...
	public DoctorMediaForHospitalService(
	...
	public MediaContentResult content(AuthenticatedActor actor, Long doctorId, Long mediaId) {
		...
		doctorRepository
			.findByIdAndHospital_IdAndDeletedAtIsNullAndHospital_DeletedAtIsNull(doctorId, hospitalId)
		...
		Media media = mediaRepository.findByIdAndDeletedAtIsNull(mediaId)
			.filter(item -> item.ownerType() == MediaOwnerType.DOCTOR)
			.filter(item -> doctorId.equals(item.ownerId()))
			.filter(item -> DOCTOR_COLLECTIONS.contains(item.collection()))
