package com.platform.common.web.partner;
...
public record PartnerOptionsReplaceRequest(
	@NotNull @Size(max = 300) List<@Valid Item> options
) {

	public ReplacePartnerOptionsCommand toCommand() {
		return new ReplacePartnerOptionsCommand(options.stream().map(Item::toCommand).toList());
	}
	...
	public record Item(
		...
		private ReplacePartnerOptionsCommand.Item toCommand() {
			return new ReplacePartnerOptionsCommand.Item(id, new SavePartnerOptionCommand(
				categoryId,
				name,
				description,
				regularPrice,
