...
반영구, 에스테틱, 헤어, 왁싱, 타투, 네일, 마사지 등 여러 뷰티 업종을 다루는 **사업화 목적의 지역 기반 업체 탐색 플랫폼**입니다. 네이버 플레이스처럼 사용자가 지역과 업종으로 업체를 찾되, 시술·가격·전문가·작업 이미지처럼 뷰티 서비스 선택에 필요한 정보를 한곳에서 비교하고 업체별 이벤트를 확인·신청한 뒤 상담과 예약까지 이어지는 구조를 목표로 개발하고 있습니다.
...
일반적인 장소 검색은 위치, 영업시간과 후기 확인에는 유용하지만 뷰티 서비스를 결정할 때 필요한 시술 종류, 가격, 담당 전문가와 작업 결과, 현재 진행 중인 이벤트를 같은 기준으로 비교하기 어렵습니다. 이 프로젝트는 장소 정보를 보여주는 데서 끝나지 않고 업체 비교, 이벤트 신청, 상담과 예약 결정에 필요한 정보를 업종별 구조로 관리하는 것을 목표로 합니다.

```text
사용자
  지역·업종 검색 -> 업체 비교 -> 서비스·전문가·이벤트 확인 -> 신청·상담 -> 예약

파트너 업체
  입점·계정 연결 -> 매장·서비스·전문가·미디어·이벤트 관리 -> 신청·예약 대응

플랫폼 운영자
  업체 등록·검수 -> 담당자 지정 -> 파트너 온보딩 -> 상태·정보 품질 관리
```

첫 번째 개발 단계는 사용자 화면을 먼저 넓히는 대신, 검색 결과의 기반이 되는 업체·서비스·전문가 데이터를 일관되게 수집하고 검수할 수 있는 운영 체계를 만드는 것입니다. 이후 업체별 이벤트와 신청 내역, 사용자 탐색, 상담·예약을 연결하고 실제 이용 데이터가 쌓이면 후기와 추천 기능으로 확장할 계획입니다.
...
| 이벤트·신청 | 미구현 | 업체별 프로모션과 사용자 신청 흐름 |
| 상담·예약 | 미구현 | 신청 이후 상담과 일정 확정 흐름 |
| 후기·채팅·알림 | 미구현 | 이용 경험과 커뮤니케이션 확장 단계 |
